...
class WebhookController extends Controller
{
    ...
    /**
     * Marquer une conversation comme transférée (Chatwoot)
     * 
     * POST /api/webhook/transfer
     */
    public function handleTransfer(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'session_id' => 'required|string',
        ...
        try {
            ...
            $conversation->chatwoot_conversation_id = $request->chatwoot_conversation_id;
            $conversation->chatwoot_contact_id = $request->chatwoot_contact_id;
            $conversation->transferred_at = now();
            $conversation->save();

            // Logger l'événement de transfert
            ConversationEvent::logAgentTransfer(
                $conversation,
                $conversation->last_widget ?? 'unknown',
                $request->transfer_reason,
                [
                    'chatwoot_conversation_id' => $request->chatwoot_conversation_id,
                    'chatwoot_contact_id' => $request->chatwoot_contact_id
                ]
            );

            // Mettre à jour les stats
            DailyStatistic::today()->increment('transferred_conversations');
