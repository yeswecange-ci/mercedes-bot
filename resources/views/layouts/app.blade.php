    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg sidebar-transition lg:translate-x-0 flex flex-col"
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto custom-scrollbar">
            ...
            <a href="{{ route('dashboard.pending') }}"
               ...
               x-init="setInterval(() => {
                ...
                <span class="flex-1 truncate">En attente agent</span>
                <span x-show="pendingCount > 0"
                      x-transition
                      class="ml-auto flex-shrink-0 px-2 py-0.5 text-xs font-bold rounded-full pulse-badge @if(request()->routeIs('dashboard.pending')) bg-white text-orange-600 @else bg-red-600 text-white @endif"
            ...
            <a href="{{ route('dashboard.active') }}"
               class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors duration-150 @if(request()->routeIs('dashboard.active')) bg-green-600 text-white shadow-sm @else text-gray-700 hover:bg-gray-100 @endif">
                ...
                <span class="truncate">Conversations actives</span>
            </a>
            ...
            <a href="{{ route('dashboard.conversations') }}"
               class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors duration-150 @if(request()->routeIs('dashboard.conversations') || request()->routeIs('dashboard.show')) bg-blue-600 text-white shadow-sm @else text-gray-700 hover:bg-gray-100 @endif">
                ...
                <span class="truncate">Toutes les conversations</span>
            </a>
            ...
            <a href="{{ route('dashboard.search') }}"
               class="flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-colors duration-150 @if(request()->routeIs('dashboard.search')) bg-cyan-600 text-white shadow-sm @else text-gray-700 hover:bg-gray-100 @endif">
                ...
                <span class="truncate">Recherche</span>
            </a>
        ...
        <div class="border-t border-gray-200 flex-shrink-0" x-data="{ userMenuOpen: false }">
            <button @click="userMenuOpen = !userMenuOpen"
                    class="flex items-center w-full px-4 py-3 text-sm hover:bg-gray-50 transition-colors duration-150 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-blue-500">
                <div class="flex-shrink-0 w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-blue-700 flex items-center justify-center text-white font-bold text-sm shadow-sm">
            ...
            <div x-show="userMenuOpen"
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="flex items-center w-full px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors duration-150">
    ...
    <div class="lg:pl-64 min-h-screen flex flex-col">
                ...
                <div class="flex items-center space-x-2 sm:space-x-4 ml-4">
                    ...
                    <div class="relative" x-data="{ notifOpen: false }">
                        <button @click="notifOpen = !notifOpen"
                                class="relative p-2 text-gray-500 hover:text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-lg">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
                            </svg>
                            @if(isset($activeCount) && $activeCount > 0)
                            <span class="absolute top-1 right-1 block h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white pulse-badge"></span>
                            @endif
                        </button>
                                   ...
                                   class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-100 transition-colors duration-150">
                                    <div class="flex items-start">
                                        ...
                                        <div class="ml-3 flex-1 min-w-0">
                                            <p class="text-sm font-semibold text-gray-900">{{ $activeCount }} conversation(s) active(s)</p>
        ...
        <main class="flex-1 p-4 sm:p-6 max-w-[1600px] w-full mx-auto">
            ...
            <div class="mb-4 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg flex items-center shadow-sm"
                ...
                <button @click="show = false"
                        class="ml-4 flex-shrink-0 text-red-600 hover:text-red-800 focus:outline-none">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/>
                    </svg>
                </button>
